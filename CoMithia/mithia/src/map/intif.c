...
int intif_parse_userlist(int fd) {
	...
	int i,len,class,mark,clan,path;
	char name[16];
	...
	WFIFOB(sd->fd,3)=0x36;
	WFIFOB(sd->fd,4)=0x03;
	WFIFOW(sd->fd,5)=SWAP16(count); //max users
	...
	for(i=0;i<count;i++) {
		...
		memcpy(name,RFIFOP(fd,i*22+16),16);
		...
		WFIFOB(sd->fd,len+10)=(path)%256;
		WFIFOB(sd->fd,len+11)=(16*mark)+classdb_icon(class);
		WFIFOB(sd->fd,len+12)=143;
		if(sd->status.clan_id && sd->status.clan_id==clan) WFIFOB(sd->fd,len+12)=63;
		WFIFOB(sd->fd,len+13)=strlen(name);
		strcpy(WFIFOP(sd->fd,len+14),name);
		len=len+strlen(name)+4;
		}
