...
int clif_clickonplayer(USER* sd,struct block_list *bl) {
    ...
    if (!pc_isequip(tsd, EQ_ARMOR)) {
		...
		WFIFOW(sd->fd, len+6) = SWAP16(itemdb_look(pc_isequip(tsd, EQ_ARMOR)));
				
		if(tsd->status.armor_color>0) {
			WFIFOB(sd->fd,len+8)=tsd->status.armor_color;
		} else {
			WFIFOB(sd->fd,len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_ARMOR));
		}
	}
	//coat
	if(pc_isequip(tsd,EQ_COAT)) {
		WFIFOW(sd->fd,len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_COAT)));
		WFIFOB(sd->fd,len+8) = itemdb_lookcolor(pc_isequip(tsd,EQ_COAT));
    }

	len+=3;
	//weapon
	if (!pc_isequip(tsd, EQ_WEAP)) {
		WFIFOW(sd->fd, len+6) = 0xFFFF;
		WFIFOB(sd->fd, len+8) = 0;
	} else {
		WFIFOW(sd->fd, len+6) = SWAP16(itemdb_look(pc_isequip(tsd, EQ_WEAP)));
		WFIFOB(sd->fd, len+8) = itemdb_lookcolor(pc_isequip(tsd, EQ_WEAP));
	}
	len+=3;
	//shield
	if (!pc_isequip(tsd, EQ_SHIELD)) {
		...
		WFIFOW(sd->fd, len+6) = SWAP16(itemdb_look(pc_isequip(tsd, EQ_SHIELD)));
		WFIFOB(sd->fd, len+8) = itemdb_lookcolor(pc_isequip(tsd, EQ_SHIELD));
	}
	len+=3;
	...
	//beard stuff
		...
		WFIFOW(sd->fd, len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_FACEACC))); //beard num
		WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC)); //beard color
	}
	len+=3;
	//crown
	if (!pc_isequip(tsd,EQ_CROWN)) {
		...
		WFIFOB(sd->fd, len)=0;
		WFIFOW(sd->fd, len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_CROWN))); //Crown
		WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN)); //Crown color
	}
	len+=3;

	if (!pc_isequip(tsd, EQ_FACEACCTWO)) {
		WFIFOW(sd->fd, len+6)=0xFFFF; //second face acc
		WFIFOB(sd->fd,len+8)=0; //" color
	} else {
		WFIFOW(sd->fd, len+6) = SWAP16(pc_isequip(tsd, EQ_FACEACCTWO));
		WFIFOB(sd->fd, len+8) = tsd->status.equip[EQ_FACEACCTWO].custom;
	}
	...
	if (!pc_isequip(tsd,EQ_MANTLE)) {
		WFIFOW(sd->fd, len+6)=0xFFFF;
		WFIFOB(sd->fd, len+8)=0xFF;
	} else {
		WFIFOW(sd->fd, len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_MANTLE)));
		WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_MANTLE));
	}
	len+=3;
	//necklace
	if (!pc_isequip(tsd,EQ_NECKLACE)) {
		...
		WFIFOW(sd->fd, len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_NECKLACE))); //necklace
		WFIFOB(sd->fd, len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_NECKLACE)); //neckalce color
	}
	len+=3;
	//boots
	if (!pc_isequip(tsd,EQ_BOOTS)) {
		WFIFOW(sd->fd, len+6)=SWAP16(tsd->status.sex); //boots
		WFIFOB(sd->fd,len+8)=0;
	} else {
		WFIFOW(sd->fd,len+6)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_BOOTS)));
		WFIFOB(sd->fd,len+8)=itemdb_lookcolor(pc_isequip(tsd,EQ_BOOTS));
	}
	len+=3;
	...
	for (x = 0, n = 0, w = 0, r = 0, m = 0, p = 0, a = 0, g = 0; (n + w + r + m + p + a + g) < sd->group_count; x++) {
		...
		switch(classdb_path(tsd->status.class)) {
			...
			case 4:
				...
				break;
			case 5:
				sprintf(buf, "%s (A)\0", tsd->status.name);
				break;
			default:
		...
		WFIFOB(sd->fd,len+7)=strlen(buf);
		strcpy(WFIFOP(sd->fd,len+8), buf);
		len+=strlen(buf)+1;
		...
		WFIFOB(sd->fd,len+8)=tsd->status.state;
		WFIFOB(sd->fd,len+9)=tsd->status.face;
		WFIFOB(sd->fd,len+10)=tsd->status.hair;
		WFIFOB(sd->fd,len+11)=tsd->status.hair_color;
		WFIFOB(sd->fd,len+12)=0x07;
		//WFIFOB(sd->fd,len+12)=tsd->status.skin_color;
		//WFIFOW(sd->fd,len+12)=0;
		len+=6;

		if (!pc_isequip(tsd,EQ_HELM) || !(tsd->status.settingFlags & FLAG_HELM) || (itemdb_look(pc_isequip(tsd,EQ_HELM))==-1)) {
			//helm stuff goes here
			WFIFOB(sd->fd, len+7)=0; // supposed to be 1=Helm, 0=No helm
			WFIFOW(sd->fd, len+8)=0xFFFF; // supposed to be Helm num
			WFIFOB(sd->fd, len+10)=0;
		} else {
			WFIFOB(sd->fd, len+7)=1;
			WFIFOW(sd->fd, len+8)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_HELM)));
			WFIFOB(sd->fd, len+10)=itemdb_lookcolor(pc_isequip(tsd,EQ_HELM));
		}

		if (!pc_isequip(tsd,EQ_FACEACC)) {
		//beard stuff
			WFIFOW(sd->fd, len+11)=0xFFFF;
			WFIFOB(sd->fd, len+13)=0x0;
		} else {
			WFIFOW(sd->fd, len+11)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_FACEACC))); //beard num
			WFIFOB(sd->fd, len+13)=itemdb_lookcolor(pc_isequip(tsd,EQ_FACEACC)); //beard color
		}

		if (!pc_isequip(tsd,EQ_CROWN)) {
			WFIFOW(sd->fd, len+14)=0xFFFF;
			WFIFOB(sd->fd, len+16)=0x0;
		} else {
			WFIFOB(sd->fd, len+7)=0;
			WFIFOW(sd->fd, len+14)=SWAP16(itemdb_look(pc_isequip(tsd,EQ_CROWN))); //Crown
			WFIFOB(sd->fd, len+16)=itemdb_lookcolor(pc_isequip(tsd,EQ_CROWN)); //Crown color
		}
		
		if (!pc_isequip(tsd, EQ_FACEACCTWO)) {
			WFIFOW(sd->fd,len+17)=0xFFFF; //second face acc
			WFIFOB(sd->fd,len+19)=0x0; //" color
		} else {
			WFIFOW(sd->fd, len+17) = SWAP16(pc_isequip(tsd, EQ_FACEACCTWO));
			WFIFOB(sd->fd, len+19) = tsd->status.equip[EQ_FACEACCTWO].custom;
		}
...
int clif_grouphealth_update(USER *sd) {
	...
	switch(classdb_path(sd->status.class)) {
		case 0:
			sprintf(buf, "%s (N)\0", sd->status.name);
			break;
		case 1:
			sprintf(buf, "%s (W)\0", sd->status.name);
			break;
		case 2:
			sprintf(buf, "%s (R)\0", sd->status.name);
			break;
		case 3:
			sprintf(buf, "%s (M)\0", sd->status.name);
			break;
		case 4:
			sprintf(buf, "%s (P)\0", sd->status.name);
			break;
		case 5:
			sprintf(buf, "%s (A)\0", sd->status.name);
			break;
		default:
			sprintf(buf, "%s (GM)\0", sd->status.name);
			break;
	}
	...
	for (x = 0; x < sd->group_count; x++) {
		...
		WFIFOB(tsd->fd,3)=0x63;
		WFIFOB(tsd->fd,4)=0x03;
		WFIFOB(tsd->fd,5)=0x03;

		WFIFOB(tsd->fd,6)=strlen(buf);
		strcpy(WFIFOP(tsd->fd,7),buf);
		len+=strlen(buf)+1;
		WFIFOL(tsd->fd,len+6)=SWAP32(sd->status.hp);
		len+=4;
		WFIFOL(tsd->fd,len+6)=SWAP32(sd->status.mp);
		len+=4;
