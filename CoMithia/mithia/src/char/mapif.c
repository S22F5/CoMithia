...
int mapif_parse_userlist(int fd) {
	...
	if(SQL_ERROR == SqlStmt_Prepare(stmt,"SELECT `ChaPthId`, `ChaMark`, `ChaClnId`, `ChaName` FROM `Character` WHERE `ChaOnline` = 1 AND `ChaHeroes` = 1 GROUP BY ChaName ORDER BY `ChaLevel` DESC, SUM(`ChaMaximumMana` + `ChaMaximumVita`) DESC, `ChaId` ASC")
	|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	{
		WFIFOW(fd,x*22+10)=class;
		WFIFOW(fd,x*22+12)=mark;
		WFIFOW(fd,x*22+14)=clan;
		memcpy(WFIFOP(fd,x*22+16),name,16);
	}
