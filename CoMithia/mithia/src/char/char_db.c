...
int char_db_newchar(const char *name, const char *pass, int totem, int sex, int country, int face, int hair, int face_color, int hair_color) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Character` (`ChaId`,`ChaName`, `ChaPassword`, `ChaTotem`, `ChaSex`, `ChaNation`, `ChaFace`, `ChaMapID`, `ChaX`, `ChaY`, "
		"`ChaHair`, `ChaHairColor`, `ChaFaceColor`) VALUES ('%d', '%s', MD5('%s %s'), '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')",
		newid, name, strlwr(name), pass, totem, sex, country, face, start_pos.m, start_pos.x, start_pos.y, hair,hair_color,face_color)) {
			Sql_ShowDebug(sql_handle);
			return 2; //db error
	}
...
int char_db_setpass(const char *name, const char *pass, const char *newpass) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Character` SET `ChaPassword` = MD5('%s %s') WHERE `ChaName` = '%s'", strlwr(name), newpass, name))
		return -1; //db_error
