...
thrown = function(player, target)
	local multiTargetDamageMod = 9 --player.damage = player.damage * ((mod - x + 1) / mod) x = targetNumber
	local arrow = 49
	local bowRange = 8 --max range of bow
	local maxTargets = 1 --max targets to acquire and hit, will not exceed bowRange
	local mobList = {} --list of mobs, only the first is taken
	...
	repeat
		...
		if (player.side == 0) then
			...
			mobList = player:getObjectsInCell(player.m, player.x, player.y - i, BL_MOB)
		...
		elseif (player.side == 1) then
			...
			mobList = player:getObjectsInCell(player.m, player.x + i, player.y, BL_MOB)
			if (player.pvp > 0) then
	...
	if (numTargets <= 0) then
		if (player.side == 0) then
			player:throw(player.x, player.y - bowRange, 49, 0, 1)
		elseif (player.side == 1) then
			player:throw(player.x + bowRange, player.y, 50, 0, 1)
		elseif (player.side == 2) then
			player:throw(player.x, player.y + bowRange, 51, 0, 1)
		elseif (player.side == 3) then
			player:throw(player.x - bowRange, player.y, 52, 0, 1)
		end
		...
		if (#pcList > 0) then
			...
			for x = 1, numTargets - 1 do
				if (player.side == 0) then
					mobList = player:getObjectsInCell(player.m, targetList[1].x, targetList[1].y - x, BL_MOB)
				...
				elseif (player.side == 1) then
					mobList = player:getObjectsInCell(player.m, targetList[1].x + x, targetList[1].y, BL_MOB)
				...
				if (player.critChance > 0) then
					...
					if (#spells > 0) then
						for i = 1, #spells do
							...
							if (script ~= nil) then
								funcPassiveOnHit = rawget(script, "passive_on_hit")
							end
					...
					if (#durations > 0) then
						for i = 1, #durations do
							...
							if (script ~= nil) then
								funcOnHitWhileCast = rawget(script, "on_hit_while_cast")
							end
