...
on_attacked = function(mob, attacker)
	...
	if (distance(mob, attacker) == 1 and math.random(100) <= 15) then
	...
	elseif (attacker:hasDuratoin("snipe") == false) then
		if (math.random(100) <= 25) then
			...
			for i = 1, #targets do
				if (targets[i].baseClass == 3) then
					if (math.random(100) <= 35) then
						...
						local randString = math.random(#stopStrings)
						if (math.random(100) <= 5 and targets[i].state ~= 1) then
							targets[i]:msg(0, mob.name..'" '..stopStrings[randString], targets[i].ID)
						end
						if (math.random(100) <= 25 and targets[i].state ~= 1 and targets[i]:hasDuration("snipe") == false) then
