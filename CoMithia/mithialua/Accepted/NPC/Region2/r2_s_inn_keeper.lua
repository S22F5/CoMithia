...
click = async(function(player,npc)
	local t={graphic=convertGraphic(10,"monster"),color=3}
	local opts = { }
	local optsPO = {}
	...
	"Welcome "..player.name..". Would you like to rest for the night?"}
		if (player.npc["r2s_inn_keeper"] > 0) then
	...
	table.insert(opts,"Post Office Services")
		if (player.npc["r2s_inn_keeper"] == 15) then
			player.npc["r2s_inn_keeper_special_offer_timer"] = os.time()
		elseif ((player.npc["r2s_inn_keeper"] >= 20) and 
		(player.npc["r2s_inn_keeper"] <= 25) and 
		((player.npc["r2s_inn_keeper_special_offer_timer"] + 300) <= os.time()) and 
		(player:hasSpace(offer_item, 1, player.ID, ""..offer_engrave) == true) and 
		(player.money >= 200)) then
			table.insert(opts,"Special Offer")
			player.npc["r2s_inn_keeper"] = 1
		end
		
	if(#opts~=0) then
		local menuOption=player:menuString(""..welcomemsg[rwelcome],opts)
			if (player.npc["r2s_inn_keeper"] < 30) then
				player.npc["r2s_inn_keeper"] = player.npc["r2s_inn_keeper"] + 1
			elseif (player.npc["r2s_inn_keeper"] >= 30) then
				player.npc["r2s_inn_keeper"] = 1
			end
...
action = function(npc, player)
	local str = {
	--"Thimble(Tavern Keeper): We have the best sheets in the whole region.",
	--"Thimble(Tavern Keeper): Here goes another round!"
	}
