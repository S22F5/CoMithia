	...
	if (#targets > 0) then
		...
		for i = 1, #targets do
			if (targets[i].blType == BL_PC) then
				player:setAether("static_shock", aether)
			end
			--if (checkProtection(player, targets[i], 1)) then
...
requirements = function(player)
	...
	local items = {}
	local itemAmounts = {}
	local description = {"Strong elemental attack striking foes around the caster"}
