...
move = function(mob, target)
	...
	local spawn = true
	local player = nil
	local magnus = nil --capricorn
	local isgar = nil --leo
	local tanna = nil --aquarius
	local hiro = nil --cancer
	local ichkin = nil --virgo
	local arelai = nil --libra
	local panda = nil --STANDIN :o
	local corthol = nil
	...
	if (opening == 0) then
		...
		mob:msg(0, "-------------------------------------------------------", player.ID)
		mob:msg(0, "---------------------Curtain Call----------------------", player.ID)
		mob:msg(0, "-------------------------------------------------------", player.ID)
	...
	elseif (opening == 27) then
		mob:talk(3, player.name..": Who are these people? What are they doing here?")
	...
	elseif (opening == 56) then
		mob:talk(3, player.name..": Getting things ready? I'd better hide.")
		mob:moveGhost()
	...
	elseif (opening == 268) then
		corthol:talk(0, "Corthol: What's going on here? Why isn't this prepared!?")
	...
	elseif (opening == 449) then
		corthol:talk(0, "Corthol: Hurry up! Once this ritual is complete I will harness the power of these lands!")
	...
	elseif (opening == 469) then
		corthol:talk(0, "Corthol: Yes, I can feel myself absorbing the earth from beneath us.")
	...
	elseif (opening == 487) then
		...
		corthol:talk(0, "Corthol: Soon my power will be on par with that of the gods!")
	...
	elseif (opening == 569) then
		if (mob.registry["opening_counter"] >= 240) then
			...
			mob.registry["opening_counter"] = mob.registry["opening_counter"] + 1
	...
	elseif (opening == 583) then
		if (decision == 1 or decision == 3) then
			...
			mob:talk(0, player.name..": What's up fellas? Can I get in on this?")
		end
	...
	elseif (opening == 618) then
		if (decision == 1 or decision == 3) then
			corthol:talk(0, "Corthol: Enough! I will show this world the true meaning of power.")
		end
	...
	elseif (opening == 630) then
		corthol:talk(0, "Corthol: The homes and villages of those who do not worship me shall be obliterated.")
	...
	elseif (opening == 642) then
		...
		corthol:talk(0, "Corthol: I, Corthol, am now ascended and one with the Gods.")
	...
	elseif (opening == 662) then
		corthol:talk(0, "Corthol: The gods said I was unfit to become one of them, I'll show them.")
		corthol:moveGhost()
	...
	elseif (opening == 674) then
		...
		corthol:talk(0, "Corthol: They think that they are the only ones who can be all powerful.")
		corthol:moveGhost()
	...
	elseif (opening == 686) then
		corthol:talk(0, "Corthol: Simply because their bloodline determines it, bah!")
		corthol.side = 1
		corthol:sendSide()
	...
	elseif (opening == 698) then
		corthol:talk(0, "Corthol: I will bring terror to this land and then come for their throats.")
		mob.side = 0
		mob:sendSide()
	elseif (opening == 706) then
		mob:talk(3, player.name..": Damn! I can't let this happen, I can't.")
	elseif (opening == 714) then
		player:changeView(6, 14)
		mob:talk(0, player.name..": I won't let you do this, not to this land, not to the people who live here peacefully!")
	elseif (opening == 718) then
		corthol.side = 2
		corthol:sendSide()
	elseif (opening == 730) then
		corthol:talk(0, "Corthol: Who are you? How dare you even think of thwarting my plans!")
	elseif (opening == 746) then
		corthol:talk(0, "Corthol: To halt a god from their destination is to wish for certain death!")
...
decision_one = async(function(player, mob)
	local decisionOne = player:menuString("What is going on? Should I do something?", {"Wait.", "Assist.", "Attack."})
	...
	if (decisionOne == "Wait.") then
		player:dialogSeq({gfx, "I will wait and see what happens to better analyze the situation."}, 1)
		mob.registry["opening_decision_one"] = 1
		mob.registry["opening"] = 570
		player:lock()
	elseif (decisionOne == "Assist.") then
		player:dialogSeq({gfx, "I shall join him in the surge for power!"}, 1)
...
getfreemap = function()
	for i = 0, opening_scene.maxmaps - 1 do
		if ((openingSceneUsedMaps[i + 1] ~= nil and openingSceneUsedMaps[i + 1] ~= (20000 + i)) or openingSceneUsedMaps[i + 1] == nil) then
			return (i + 20000)
		end
	end
