surt_of_muspelheim = {
			...
			elseif(menuOption=="Add Kindred" and player.registry["clan"..clannumber.."kindredlevel"]>=2) then
					...
					if(string.lower(name)~=string.lower(player.name)) then
							if(Player(name).clan==0) then
								...
								surt_of_muspelheim.addKindred(Player(name),npc)
									...
									if(#nuisance>0) then
										for z=1,#nuisance do
								 			if(nuisance[z].name==nameofPlayer and nuisance[z].clan==clannumber) then
												...
												Player(nameofPlayer):addLegend("Primogen of Heroes of Aesir since Year " .. player:curYear() .. ", " .. player:curSeason() .. "","primogen",7,128)
	...
	addKindred=async(function(player,npc)
			...
			local choice=player:menuString2(""..Player(player.registry["kindredinviter"]).name.." Wishes to make you a kindred of Heroes of Aesir, do you accept?",opts)
				if(choice=="No") then
				...
				elseif(choice=="Yes") then
						...
						player:sendMinitext("You are now a Kindred of Heroes of Aesir.")
				end
