soekarno = {
	click =async(function(player,npc)
		local t={graphic=convertGraphic(480,"monster"),color=0}
			...
			elseif(menuOption=="Add Kindred" and player.registry["clan"..clannumber.."kindredlevel"]>=2) then
					...
					if(string.lower(name)~=string.lower(player.name)) then
							if(Player(name).clan==0) then
								...
								soekarno.addKindred(Player(name),npc)
									...
									if(#nuisance>0) then
										for z=1,#nuisance do
								 			if(nuisance[z].name==nameofPlayer and nuisance[z].clan==clannumber) then
												...
												Player(nameofPlayer):addLegend("Primogen of Patriot since Year " .. player:curYear() .. ", " .. player:curSeason() .. "","primogen",7,128)
	...
	addKindred=async(function(player,npc)
			...
			local choice=player:menuString2(""..Player(player.registry["kindredinviter"]).name.." Wishes to make you a kindred of Patriot, do you accept?",opts)
				if(choice=="No") then
				...
				elseif(choice=="Yes") then
						...
						player:sendMinitext("You are now a Kindred of Patriot.")
				end
