housekeeper53146 = {
	...
	click=async(function(player,npc)
		...
		if(player.registry["house53146owner"]==1 or player.registry["house53146guest"]==1) then		
		...
		if(player.mapRegistry["houserentbank"]>=os.time()) then
			...
			local choice=player:menuString("How can I help you?",mainopts)
				...
				elseif(choice=="Change PVP settings" and player.registry["house53146owner"]==1) then
						player:dialogSeq({t,"Not implemented yet."})
				elseif(choice=="Change lights" and player.registry["house53146owner"]==1) then
						player:dialogSeq({t,"Not implemented yet."})
				elseif(choice=="Rent house features" and player.registry["house53146owner"]==1) then
				...
				elseif(choice=="Nuisance") then
					local nuisance=string.lower(player:input("Who needs to be nuisanced?"))
						if(Player(nuisance).m==10008) then
							Player(nuisance):warp(53,14,8)
