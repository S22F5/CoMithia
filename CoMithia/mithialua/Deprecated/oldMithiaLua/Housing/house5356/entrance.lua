housing5356 = {
	click = async(function(player,npc)
		...
		if(player.mapRegistry["house5356setting"]==0) then
		...
		if(player.registry["house5356owner"]==1) then
			local choice=player:menuString2("What do you wish to do?",opts)
				if(choice=="Enter house" and player.registry["house5356owner"]==1) then
					player:warp(10005,17,18)
				elseif(choice=="Relinquish Ownership" and player.registry["house5356owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing5356.ownercheck(Player(name),npc)
									end
								end
							end
						end
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing5356.sellcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Invite guest" and player.registry["house5356owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing5356.guestcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Remove guest access" and player.registry["house5356owner"]==1) then
		...
		elseif(player.registry["house5356guest"]==1 or player.mapRegistry["house5356setting"]==1) then
				player:warp(10005,17,18)
		else
			player:warp(53,5,8)
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseownership"]).name)) then
										check[z].registry["house5356owner"]=0
										check[z].registry["house5356guest"]=0
										player.registry["house5356owner"]=1
										player.registry["house5356guest"]=0
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseguest"]).name)) then
										player.registry["house5356guest"]=1
