housing53116 = {
	click = async(function(player,npc)
		...
		if(player.mapRegistry["house53116setting"]==0) then
		...
		if(player.registry["house53116owner"]==1) then
			local choice=player:menuString2("What do you wish to do?",opts)
				if(choice=="Enter house" and player.registry["house53116owner"]==1) then
					player:warp(10007,15,30)
				elseif(choice=="Relinquish Ownership" and player.registry["house53116owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing53116.ownercheck(Player(name),npc)
									end
								end
							end
						end
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing53116.sellcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Invite guest" and player.registry["house53116owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing53116.guestcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Remove guest access" and player.registry["house53116owner"]==1) then
		...
		elseif(player.registry["house53116guest"]==1 or player.mapRegistry["house53116setting"]==1) then
				player:warp(10007,15,30)
		else
			player:warp(53,11,8)
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseownership"]).name)) then
										check[z].registry["house53116owner"]=0
										check[z].registry["house53116guest"]=0
										player.registry["house53116owner"]=1
										player.registry["house53116guest"]=0
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseguest"]).name)) then
										player.registry["house53116guest"]=1
