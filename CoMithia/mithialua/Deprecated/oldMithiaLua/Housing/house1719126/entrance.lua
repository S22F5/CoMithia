housing1719127 = {
	click = async(function(player,npc)
		...
		if(player.mapRegistry["house1719127setting"]==0) then
		...
		if(player.registry["house1719127owner"]==1) then
			local choice=player:menuString2("What do you wish to do?",opts)
				if(choice=="Enter house" and player.registry["house1719127owner"]==1) then
					player:warp(10002,8,13)
				elseif(choice=="Relinquish Ownership" and player.registry["house1719127owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing1719127.ownercheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Set House Entrance : Private" and player.registry["house1719127owner"]==1) then
					player:dialogSeq({"This tool allows you to close your house to the Public (Non guests). Click next to proceed."},1)
					player.mapRegistry["house1719127setting"]=0
					player:sendMinitext("Your house is now set on private.")
				elseif(choice=="Set House Entrance : Public" and player.registry["house1719127owner"]==1) then
					player:dialogSeq({"This tool allows you to open your house to the Public (Non guests). Click next to proceed."},1)
					player.mapRegistry["house1719127setting"]=1
					player:sendMinitext("Your house is now set on public.")
				elseif(choice=="Sell house" and player.registry["house1719127owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing1719127.sellcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Invite guest" and player.registry["house1719127owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing1719127.guestcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Remove guest access" and player.registry["house1719127owner"]==1) then
					...
					if(string.lower(name)~=string.lower(player.name)) then
						if(Player(name).registry["house1719127guest"]==1) then
						Player(name).registry["house1719127guest"]=0
						...
						if(Player(name).m==10002) then
							Player(name):warp(17,19,129)
						end
						end
					else
		...
		elseif(player.registry["house1719127guest"]==1 or player.mapRegistry["house1719127setting"]==1) then
				player:warp(10002,8,13)
		else
			player:warp(17,19,129)
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseownership"]).name)) then
										check[z].registry["house1719127owner"]=0
										check[z].registry["house1719127guest"]=0
										player.registry["house1719127owner"]=1
										player.registry["house1719127guest"]=0
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseguest"]).name)) then
										player.registry["house1719127guest"]=1
