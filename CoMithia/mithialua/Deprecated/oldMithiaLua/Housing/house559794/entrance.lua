housing559794 = {
	click = async(function(player,npc)
		...
		if(player.mapRegistry["house559794setting"]==0) then
			table.insert(opts,"Set House Entrance : Public")
		elseif(player.mapRegistry["house559794setting"]==1) then
		...
		if(player.registry["house559794owner"]==1) then
			local choice=player:menuString2("What do you wish to do?",opts)
				if(choice=="Enter house" and player.registry["house559794owner"]==1) then
					player:warp(57,8,48)
				elseif(choice=="Relinquish Ownership" and player.registry["house559794owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing559794.ownercheck(Player(name),npc)
									end
								end
							end
						end
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing559794.sellcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Invite guest" and player.registry["house559794owner"]==1) then
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(name)) then
										...
										housing559794.guestcheck(Player(name),npc)
									end
								end
							end
						end
				...
				elseif(choice=="Remove guest access" and player.registry["house559794owner"]==1) then
					...
					if(string.lower(name)~=string.lower(player.name)) then
						if(Player(name).registry["house559794guest"]==1) then
						Player(name).registry["house559794guest"]=0
		...
		elseif(player.registry["house559794guest"]==1 or player.mapRegistry["house559794setting"]==1) then
				player:warp(57,8,48)
		else
			player:warp(55,97,96)
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseownership"]).name)) then
										check[z].registry["house559794owner"]=0
										check[z].registry["house559794guest"]=0
										player.registry["house559794owner"]=1
										player.registry["house559794guest"]=0
							...
							if(#check>0) then
								for z=1,#check do
									if(string.lower(check[z].name)==string.lower(Player(player.registry["popuphouseguest"]).name)) then
										player.registry["house559794guest"]=1
