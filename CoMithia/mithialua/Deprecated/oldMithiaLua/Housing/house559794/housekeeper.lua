	...
	click=async(function(player,npc)
		...
		if(player.mapRegistry["houserentbank"]>=os.time()) then
			...
			local choice=player:menuString("How can I help you?",mainopts)
				...
				elseif(choice=="Change PVP settings" and player.registry["house559794owner"]==1) then
						player:dialogSeq({t,"Not implemented yet."})
				elseif(choice=="Change lights" and player.registry["house559794owner"]==1) then
						player:dialogSeq({t,"Not implemented yet."})
				elseif(choice=="Rent house features" and player.registry["house559794owner"]==1) then
				...
				elseif(choice=="Nuisance") then
					local nuisance=player:input("Who needs to be nuisanced?")
						if(Player(nuisance).m==57) then
							Player(nuisance):warp(55,97,96)
