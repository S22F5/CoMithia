	...
	open=function(player)
		if(player.side==0) then
    	        	if(player.registry["carnagehost"]>0) then		
                		local obj=getObject(2100,9,32)
				if(obj==340) then
					setObject(2100,9,32,360)
					setObject(2100,10,32,361)
					player:sendMinitext("Opened.")
					return
				elseif(obj==360) then
					setObject(2100,9,32,340)
					setObject(2100,10,32,341)
...
hid21001033 = {
	open=function(player)
		if(player.side==0) then
    	        	if(player.registry["carnagehost"]>0) then		
                		local obj=getObject(2100,10,32)
				if(obj==341) then
					setObject(2100,9,32,360)
					setObject(2100,10,32,361)
					player:sendMinitext("Opened.")
