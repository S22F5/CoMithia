azlof_assault = {
	click=async(function(player,npc)
		...
		table.insert(opts,"Enter Assault")
		...
		local menuOption=player:menuString("Good day to you, "..player.name.."",opts)
				  ...
				  if(menuOption=="Deposit item") then
                                  ...
                                  elseif(menuOption=="Enter Assault") then
					player:dialogSeq({t,"Welcome, "..player.name..".\n\nBy entering the Domination, you agree to obey the hosts and follow all the rules.",t,"The participation fee is 1000 coins, click Next if you wish to enter the Assault"},1)
					if(player.money>=1000) then
						...
						player.registry["assaultparticipations"]=player.registry["assaultparticipations"]+1
						player:removeLegendbyName("assaultparticipations")
						player:addLegend("Participated in "..player.registry["assaultparticipations"].." Assaults","assaultparticipations",1,51)
