azlof_domination = {
	click=async(function(player,npc)
		...
		table.insert(opts,"Enter Domination")
		...
		local menuOption=player:menuString("Good day to you, "..player.name.."",opts)
				  ...
				  if(menuOption=="Deposit item") then
                                  ...
                                  elseif(menuOption=="Enter Domination") then
					player:dialogSeq({t,"Welcome, "..player.name..".\n\nBy entering the Domination, you agree to obey the hosts and follow all the rules.",t,"The participation fee is 1000 coins, click Next if you wish to enter the Domination"},1)
					if(player.money>=1000) then
						...
						player.registry["dominationparticipations"]=player.registry["dominationparticipations"]+1
						player:removeLegendbyName("dominationparticipations")
						player:addLegend("Participated in "..player.registry["dominationparticipations"].." Dominations","dominationparticipations",1,51)
