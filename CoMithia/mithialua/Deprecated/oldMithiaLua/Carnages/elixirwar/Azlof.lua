azlof_elixir = {
	click=async(function(player,npc)
		...
		table.insert(opts,"Enter Elixir war")
		...
		local menuOption=player:menuString("Good day to you, "..player.name.."",opts)
				  ...
				  if(menuOption=="Deposit item") then
                                  ...
                                  elseif(menuOption=="Enter Elixir war") then
					player:dialogSeq({t,"Welcome, "..player.name..".\n\nBy entering the Elixir war, you agree to obey the hosts and follow all the rules.",t,"The participation fee is 1000 coins, click Next if you wish to enter the Elixir war"},1)
					if(player.money>=1000) then
						...
						player.registry["elixirwarparticipations"]=player.registry["elixirwarparticipations"]+1
						player:removeLegendbyName("elixirwarparticipations")
						player:addLegend("Participated in "..player.registry["elixirwarparticipations"].." Elixir wars","elixirwarparticipations",1,51)
