oak_tree = {
	...
	on_death=function(mob,player)
		...
		if(player:hasEquipped({"sturdy_woodcutting_axe"})) then
				player:sendMinitext("Your axe is not good enough for wood of that quality.")
				return
		end
	...
	if(m>r) then
		...
		s=s+b
		player:addItem("oak_log",s)
			if(t>2) then
				player.registry["woodcutter"]=player.registry["woodcutter"]+(s*3)
			end
