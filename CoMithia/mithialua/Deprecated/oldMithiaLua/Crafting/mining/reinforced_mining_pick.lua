		...
		l=2000000000/z
			...
			if(#f>0) then
				...
				if(f[1].name=="Copper vein") then
					if(player.registry["miner"]>=2200) then
						f[1]:removeHealth(l)
					else
						player:sendMinitext("You are not experienced enough to mine Copper veins.")
					end
				end
				if(f[1].name=="Silver vein") then
					if(player.registry["miner"]>=6400) then
						f[1]:removeHealth(l)
					else
						player:sendMinitext("You are not experienced enough to mine Silver veins.")
					end
				end
				if(f[1].name=="Iron vein") then
					if(player.registry["miner"]>=18000) then
						...
						player:sendMinitext("You are not experienced enough to mine Iron veins.")
					end
				end
				if(f[1].name=="Gold vein") then
					if(player.registry["miner"]>=50000) then
						f[1]:removeHealth(l)
					else
						player:sendMinitext("You are not experienced enough to mine Gold veins.")
					end
				end
			end
	end
