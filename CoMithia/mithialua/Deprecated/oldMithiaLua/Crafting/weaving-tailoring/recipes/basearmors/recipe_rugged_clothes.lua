recipe_rugged_clothes = {
	...
	if(player.registry["reciperuggedclothes"]==1) then
			player:sendMinitext("You already how to forge Rugged Clothes.")
			return
	end

	player:dialogSeq({t,"Rugged Clothes:\n\nA standard mage armor.",t,"Requires:\n\n-Adept tailor\n-10 Perendale fabric\n-1 Shetland fabric\n-1200 coins\n\nto be forged."},1)
	local choice=player:menuString2("Do you wish to learn how to tailor Rugged Clothes?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
		if(player:hasItem("recipe_rugged_clothes",1)) then
			player.registry["reciperuggedclothes"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipetailoring"]=player.registry["recipetailoring"]+1
			player:removeItem("recipe_rugged_clothes",1)
			player:dialogSeq({t,"You now know how to tailor Rugged Clothes."})
		end
		end	
	end)
}