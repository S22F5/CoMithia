recipe_hardened_clothes = {
	...
	if(player.registry["recipehardenedclothes"]==1) then
			player:sendMinitext("You already how to tailor Hardened Clothes.")
			return
	end

	player:dialogSeq({t,"Hardened Clothes:\n\nA standard mage armor.",t,"Requires:\n\n-Apprentice tailor\n-8 Wool fabric\n-2 Romney fabric\n-75 coins\n\nto be forged."},1)
	local choice=player:menuString2("Do you wish to learn how to tailor Hardened Clothes?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
			player.registry["recipehardenedclothes"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipetailoring"]=player.registry["recipetailoring"]+1
			player:removeItem("recipe_hardened_clothes",1)
			player:dialogSeq({t,"You now know how to tailor Hardened Clothes."})
		end	
	end)
}