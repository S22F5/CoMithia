recipe_fortified_mantle = {
	...
	if(player.registry["recipefortifiedmantle"]==1) then
			player:sendMinitext("You already how to tailor Fortified Mantle.")
			return
	end

	player:dialogSeq({t,"Fortified Mantle:\n\nA standard poet Mantle.",t,"Requires:\n\n-Apprentice tailor\n-10 Wool fabric\n-4 Romney fabric\n-500 coins\n\nto be forged."},1)
	local choice=player:menuString2("Do you wish to learn how to tailor a Fortified Mantle?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
			player.registry["recipefortifiedmantle"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipetailoring"]=player.registry["recipetailoring"]+1
			player:removeItem("recipe_fortified_mantle",1)
			player:dialogSeq({t,"You now know how to tailor Fortified Mantle."})
		end	
	end)
}