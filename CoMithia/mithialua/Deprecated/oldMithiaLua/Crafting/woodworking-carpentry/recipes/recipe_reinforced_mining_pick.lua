recipe_reinforced_mining_pick = {
	...
	if(player.registry["recipereinforcedminingpick"]==1) then
			player:sendMinitext("You already know how to carve a Reinforced mining pick.")
			return
	end

	player:dialogSeq({t,"Reinforced mining pick:\n\nA mining pick for T3 and below.",t,"Requires:\n\n-Talented Carpenter\n-20 Cedar board\n-20 Copper bars\n-5 Soulstones\n-1000 coins\n\nto be carved."},1)
	local choice=player:menuString2("Do you wish to learn how to carve a Reinforced mining pick?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
			if(player:hasItem("recipe_reinforced_mining_pick",1)) then
			player.registry["recipereinforcedminingpick"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipecarpenting"]=player.registry["recipecarpenting"]+1
			player:removeItem("recipe_reinforced_mining_pick",1)
