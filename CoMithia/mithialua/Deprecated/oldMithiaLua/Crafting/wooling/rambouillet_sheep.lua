rambouillet_sheep = {
	...
	on_death=function(mob,player)
		...
		if(player:hasEquipped({"basic_shears"})) then
				player:sendMinitext("Your shears are not good enough for wool of that quality.")
				return
		end
		if(player:hasEquipped({"sturdy_shears"})) then
				player:sendMinitext("Your shears are not good enough for wool of that quality.")
				return
		end
	...
	if(m>r) then
		...
		s=s+b
		player:addItem("rambouillet_wool",s)
		--Skill bonus from Tier3
		s=s*3
		--Skill bonus from Tier3
			if(t>1) then
				player.registry["shepherd"]=player.registry["shepherd"]+s*2
			end
		...
		player:addItem("rambouillet_wool",1)
	end			
	end
