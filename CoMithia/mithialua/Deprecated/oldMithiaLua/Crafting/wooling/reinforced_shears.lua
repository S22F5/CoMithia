		...
		l=2000000000/x
			...
			if(#f>0) then
				...
				if(f[1].name=="Merino sheep") then
					if(player.registry["shepherd"]>=18000) then
						f[1]:removeHealth(l)
					else
						player:sendMinitext("You are not experienced enough to shear a Merino sheep.")
					end
				end
				if(f[1].name=="Rambouillet sheep") then
					if(player.registry["shepherd"]>=50000) then
						f[1]:removeHealth(l)
					else
						player:sendMinitext("You are not experienced enough to shear a Rambouillet sheep.")
					end
				end
			end
	end
