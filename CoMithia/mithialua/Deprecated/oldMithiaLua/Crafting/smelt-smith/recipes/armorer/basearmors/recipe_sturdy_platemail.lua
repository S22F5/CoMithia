recipe_sturdy_platemail = {
	...
	if(player.registry["recipesturdyplatemail"]==1) then
			player:sendMinitext("You already how to forge a Sturdy Platemail.")
			return
	end
	...
	local choice=player:menuString2("Do you wish to learn how to forge a Sturdy Armor?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
			player.registry["recipesturdyplatemail"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipearmorsmith"]=player.registry["recipearmorsmith"]+1
			player:removeItem("recipe_sturdy_platemail",1)
			player:dialogSeq({t,"You now know how to forge a Sturdy Platemail."})
		end	
	end)
}