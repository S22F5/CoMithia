recipe_hardened_armor = {
	...
	if(player.registry["recipehardenedarmor"]==1) then
			player:sendMinitext("You already how to forge a Hardened Armor.")
			return
	end

	player:dialogSeq({t,"Hardened Armor:\n\nA standard rogue armor.",t,"Requires:\n\n-Apprentice Armorer\n-8 Tin bars\n-2 Bronze bars\n-75 coins\n\nto be forged."},1)
	local choice=player:menuString2("Do you wish to learn how to forge a Hardened Armor?\n((This action is definitive and will consume your recipe))",opts)
		if(choice=="Yes") then
			player.registry["recipehardenedarmor"]=1
			player.registry["totalrecipenumber"]=player.registry["totalrecipenumber"]+1
			player.registry["recipearmorsmith"]=player.registry["recipearmorsmith"]+1
			player:removeItem("recipe_hardened_armor",1)
			player:dialogSeq({t,"You now know how to forge a Hardened Armor."})
		end	
	end)
}