...
cast = function(player)
	...
	player:setAether("fighters_rage", aether)
	player.magic = player.magic - magicCost
	player:sendStatus()
	player:setDuration("fighters_rage", duration)
	player:playSound(35)
	player:sendAnimation(214, 0)
	player:calcStat()
	player:sendMinitext("You cast Fighter's Rage.")
end,

recast = function(player)
	player.rage = player.rage + .67
end,
...
requirements = function(player)
	...
	local items = {}
	local itemAmounts = {}
	local description = {"Hones your rage into a moderate damage boost"}
