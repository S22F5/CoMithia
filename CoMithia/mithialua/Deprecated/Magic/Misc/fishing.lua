fishing = {
		...
		local t={graphic=convertGraphic(948,"monster"),color=13}
		...
		if(target.blType==BL_MOB and target.name=="Fish") then
			...
			if(player.registry["fisherman"]>=0 and player.registry["fisherman"]<=25) then
				if(ra>=0 and ra<=20) then
					...
					player:dialogSeq({t,"You pull on your pole too hard and end up eviscerating your catch."})
				elseif(ra>20 and ra<=70) then
	                		target.attacker=player.ID
					target:removeHealth(350000)
					player:dialogSeq({t,"You pull on you pole as your catch bites, but only end up hurting it."})
				elseif(ra>70 and ra<=95) then
					...
					if(choice=="Pull harder") then
						if(ra2<=80) then
				        		target.attacker=player.ID
							target:removeHealth(350000)
							player:dialogSeq({t,"You pull too hard on your pole and hurt your catch."})
						elseif(ra2>80) then
							target.attacker=player.ID
							...
							player:dialogSeq({t,"You manage to catch it!"},1)
						end
					else
						target.attacker=player.ID
